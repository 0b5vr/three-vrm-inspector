  ...
  return (
    <Pane {...params}>
      <PaneRoot>
        <NameValueEntry
          name="Buffer"
          value={bytesToDisplayBytes(info?.memory?.buffer ?? 0.0)}
        />
        <NameValueEntry
          name="Texture"
          value={bytesToDisplayBytes(info?.memory?.texture ?? 0.0)}
        />
        <NameValueEntry
          name="Render Buffer"
          value={bytesToDisplayBytes(info?.memory?.renderbuffer ?? 0.0)}
        />
        <NameValueEntry
          name="Drawing Buffer"
          value={bytesToDisplayBytes(info?.memory?.drawingbuffer ?? 0.0)}
        />
        <NameValueEntry
          name="Total"
          value={bytesToDisplayBytes(info?.memory?.total ?? 0.0)}
        />
        <Hr />
        <NameValueEntry name="Buffer" value={info?.resources?.buffer ?? 0} />
        <NameValueEntry
          name="Render Buffer"
          value={info?.resources?.renderbuffer ?? 0}
        />
        ...
        <NameValueEntry
          ...
          value={info?.resources?.transformFeedback ?? 0}
        />
        <NameValueEntry
          name="Vertex Array"
          value={info?.resources?.vertexArray ?? 0}
        />
      </PaneRoot>
    </Pane>
  );
